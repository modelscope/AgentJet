...
from agentscope_tuner.context_tracker.agentscope_tracker.timeline_merging import (
    merge_tracker_timelines,
)
from agentscope_tuner.context_tracker.basic_tracker import (
    BaseContextTracker,
    ExtendedMessage,
)
from agentscope_tuner.schema.extended_msg import INVALID_LOG_PROB_VALUE
from agentscope_tuner.schema.trajectory import Reward
from agentscope_tuner.utils.color_hsl import adjust_color_hsl
from agentscope_tuner.utils.compute_madness import compute_string_madness
from agentscope_tuner.utils.tokenizer import astune_apply_chat_template
...
class MultiAgentContextTracker(BaseContextTracker):
    ...
    def step_prepare(self, messages: List[dict], tools: List = []):
        ...
        disable_toolcalls = self.config.astuner.rollout.agentscope_disable_toolcalls
    ...
    def step_track(
        ...
        if context_safe:
            ...
            if length > self.config.astuner.rollout.max_model_len:
                raise RuntimeError(
    ...
    def check_context_token_num_safe(
        ...
        elif self.already_mad_flag and self.config.astuner.rollout.agent_madness_termination:
