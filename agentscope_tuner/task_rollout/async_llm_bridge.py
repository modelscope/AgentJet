...
from agentscope_tuner.context_tracker.agentscope_tracker.multiagent_tracking import (
    MultiAgentContextTracker,
)
from agentscope_tuner.schema.logprob import TokenAndProb
from agentscope_tuner.utils.async_utils import run_async_coroutine_with_timeout
from agentscope_tuner.utils.testing_utils import _mock_if_test_mode, _test_if_test_mode
...
class AsyncLlmBridge(object):
    ...
    def get_llm_inference_fn(self, sampling_params: dict = {}) -> Callable:  # noqa: C901
        def llm_chat(
            ...
            if self.config.astuner.rollout.name == "vllm":
                token_array = final_res.outputs[0].token_ids
            elif self.config.astuner.rollout.name == "sglang":
            ...
            if (
                ...
                and (not self.config.astuner.rollout.agentscope_disable_toolcalls)
            ):
    ...
    async def __call__(
        ...
        **kwargs,
        ...
        if not context_safe:
            ...
            if token_overflow:
                # astune_action_when_overflow = self.config.astuner.rollout.astune_action_when_overflow
