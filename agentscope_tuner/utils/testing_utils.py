...
def _test_if_test_mode(key, value, config):
    from agentscope_tuner.backbone.warm_up import init_parallel_rollout_logger
...
def update_benchmark_status(status, status_detail, append_log="", data_dashboard_url=""):
    if "ASTUNER_GIT_HASH" not in os.environ:
        raise RuntimeError(
            "ASTUNER_GIT_HASH not found in environment variables. Please set `astuner.execute_test=False`."
        )
