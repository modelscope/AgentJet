...
from agentscope_tuner.utils.config_computer import split_keys_and_operators
...
def read_astune_hierarchical_config(
        ...
        config = {
            ...
            "hydra": {
                "searchpath": [
                    ...
                    "file://agentscope_tuner/default_config/verl",
                    "file://agentscope_tuner/default_config/trinity",
                ]
            },
            "defaults": [
                "verl_default",
    ...
    if backbone == "debug":
        if "trinity_default" in config["defaults"]:
            ...
            config["hydra"]["searchpath"].remove("file://agentscope_tuner/default_config/trinity")
    # remove extra config of verl for trinity
    if backbone == "trinity":
        if "verl_default" in config["defaults"]:
            config["defaults"].remove("verl_default")
            config["hydra"]["searchpath"].remove("file://agentscope_tuner/default_config/verl")
