#!/usr/bin/env bash
# e2b ATB coding-agent RL — swarm client 启动脚本 (8861).
# 前置 (在专用 aimux session 里, 先 source .venv 再提 ulimit):
#   1) gw_tunnel session:  ssh -N -L 12928:47.76.255.52:29928 root@10.29.255.115
...
# 网络: 沙箱回连本机 + judge 走网关隧道
export ADAPTER_PUBLIC_HOST="${ADAPTER_PUBLIC_HOST:-10.56.15.66}"
export JUDGE_MODEL_SERVER="${JUDGE_MODEL_SERVER:-http://localhost:12928}"

# coding-agent-material 路径 + 二进制
export E2B_ATBENCH_MATERIAL="$MAT"
export SLIME_ROOT="$MAT"
...
export CC_ANTHROPIC_AUTH_TOKEN="${CC_ANTHROPIC_AUTH_TOKEN:-sk-wefjoewfewhviuwhoewjfoiwehfiuewhvbdjnasjcoqjfdow}"
...
echo "=== e2b_atbench swarm client (8861) ==="
...
echo "  material: $MAT   judge_gateway: $JUDGE_MODEL_SERVER"
echo "------------------------------------------------------"
