...
class E2BSandbox:
    ...
    async def __aenter__(self) -> E2BSandbox:
        ...
        # Template 模式 (PAI-EAS 等用 template 路由的网关): 设 SLIME_AGENT_E2B_TEMPLATE
        # 则用 AsyncSandbox.create(template=...) 绕过 metadata 路由。
        template = _getenv("SLIME_AGENT_E2B_TEMPLATE", "E2B_TEMPLATE") or None
        if template is not None:
            self._sb = await self._rpc_retry(
                "create", lambda: AsyncSandbox.create(template=template, timeout=self.timeout)
            )
    ...
    async def _wait_until_ready(self, max_attempts: int = 60, interval: float = 3.0) -> None:
        """PAI-EAS 等网关创建后沙盒需要就绪时间; is_running 在 not-ready 时抛异常, 需重试。"""
        import asyncio as _asyncio
        for i in range(1, max_attempts + 1):
            try:
                if await self._sb.is_running():
                    return
            except Exception:
                pass
            await _asyncio.sleep(interval)
        logger.warning("[agent.sandbox] sandbox %s not ready after %d polls", self._sb.sandbox_id[:8], max_attempts)
