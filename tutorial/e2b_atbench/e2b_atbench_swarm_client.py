...
# ── 用户指定参数 ─────────────────────────────────────────────
REMOTE_BATCH_SIZE = 4      # remote batch size
NUM_REPEAT = 4             # num_repeat (GRPO 组大小)
MAX_PARALLEL = 16          # 并行最大 episode 数
# ────────────────────────────────────────────────────────────
NUM_EPOCH = 10000
...
REMOTE_NNODES = int(os.getenv("REMOTE_NNODES", "2"))   # 8861 + 8862 = 16 卡
...
REMOTE_PPO_MAX_TOKEN_LEN_PER_GPU = int(os.getenv("PPO_MAX_TOKEN_LEN_PER_GPU", "100000"))
...
ajet_job = AgentJetJob(
    ...
    # 用户指定: 100k 上下文, 单轮 4k, 总 response 96k, prompt 4k
    max_prompt_length=4096,
    max_response_length=96000,
    max_model_len=102400,   # >= prompt(4096)+response(96000)=100096
