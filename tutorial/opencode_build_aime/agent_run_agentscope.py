# -*- coding: utf-8 -*-
"""
AIME Math Agent (AgentScope variant).

Same multi-turn Python-code-execution agent as `agent_run_v3.py`, but the LLM call
goes through `agentscope.model.OpenAIChatModel` (parsing AgentScope `ChatResponse`
content blocks) instead of the OpenAI SDK. The Python sandbox tool, tool schema, and
reward function are reused as-is from `agent_run_v3.py`, so the only difference vs the
baseline is the client framework that talks to the AgentJet swarm.

Exposes `execute_agent(task, api_baseurl_key, ajet_job) -> WorkflowOutput`.
"""
...
class AgentLoop:
    """Mirror of agent_run_v3.AgentLoop, issuing the chat-completion via AgentScope."""
    ...
    async def _chat_completion(self, messages: list[dict], max_tokens: int):
        """AgentScope chat-completion. Returns (content, tool_calls, total_tokens)."""
        resp = await self.model_client(
            messages=messages,
            tools=self.tool_schemas if self.tool_schemas else None,
            tool_choice="auto" if self.tool_schemas else None,
            max_tokens=max_tokens,
        )
        blocks = _block_get(resp, "content", []) or []
        ...
        for b in blocks:
            ...
            if btype == "text":
                ...
                tool_calls.append({
                    ...
                    "arguments": _block_get(b, "input") or {},
                })
        usage = _block_get(resp, "usage")
        if usage is None:
            total = 0
        else:
            total = _block_get(usage, "total_tokens")
            if not total:
                total = (_block_get(usage, "input_tokens", 0) or 0) + (_block_get(usage, "output_tokens", 0) or 0)
        return "".join(text_parts), tool_calls, int(total or 0)
    ...
    async def run(self, messages: list[dict], sampling_params: dict) -> tuple[str, list[dict], int]:
        ...
        while True:
            ...
            content, tool_calls, used = await self._chat_completion(formatted_messages, max_tokens)
...
def execute_agent(
    ...
    model_client = OpenAIChatModel(
        model_name=ajet_job.model,
        api_key=api_key,
        stream=False,
        client_args={"base_url": base_url, "timeout": 300},
    )
