# -*- coding: utf-8 -*-
"""
AIME Math Agent (LangChain variant).

Same multi-turn Python-code-execution agent as `agent_run_v3.py`, but the LLM call
goes through `langchain_openai.ChatOpenAI` (with `bind_tools` and LangChain message
objects) instead of the OpenAI SDK. The Python sandbox tool, tool schema, and reward
...
class AgentLoop:
    """Mirror of agent_run_v3.AgentLoop, issuing the chat-completion via LangChain."""
    ...
    def _chat_completion(self, messages: list[dict], max_tokens: int):
        """LangChain chat-completion. Returns (content, tool_calls, total_tokens)."""
        llm = ChatOpenAI(
            model=self.model,
            ...
            timeout=300,
        ).bind_tools(self.tool_schemas)

        ai_msg = llm.invoke(_to_langchain_messages(messages))

        content = ai_msg.content if isinstance(ai_msg.content, str) else ""
        if not content and isinstance(ai_msg.content, list):
            content = "".join(b.get("text", "") for b in ai_msg.content if isinstance(b, dict))

        tool_calls = []
        for tc in (ai_msg.tool_calls or []):
            tool_calls.append({"id": tc.get("id"), "name": tc.get("name"), "arguments": tc.get("args") or {}})
