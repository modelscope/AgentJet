...
EVAL_INTERVAL = 10
EVAL_K = 2
DEFAULT_PROJECT_NAME = "subject11_aime_minimum_stable_batchsize"
...
def create_ajet_job(
    ...
    ajet_job.config.ajet.execute_test = False
    ajet_job.config.ajet.interchange_server["interchange_server_port"] = extract_swarm_port(swarm_url)
    ajet_job.config.ajet.trainer_common.test_freq = EVAL_INTERVAL
    ajet_job.config.ajet.trainer_common.save_freq = 10**9
    ajet_job.config.ajet.trainer_common.total_epochs = 10000
    ajet_job.config.ajet.trainer_common.val_pass_n = EVAL_K
    # Swarm mode cannot enable val_before_train, so the script runs an explicit step-0 eval instead.
    ajet_job.config.ajet.trainer_common.val_before_train = False
    return ajet_job
...
class AIMEAutoResearchTrainer:
    def __init__(
        ...
        self.ajet_job = None
        self.swarm_worker = None
        self.dataset = None
        self.eval_tasks_by_set = {}
        self.grpo_n = 8
    ...
    def setup(self):
        ...
        self.ajet_job = create_ajet_job(
            batch_size=self.batch_size,
            max_response_length_in_one_turn=self.max_response_length_in_one_turn,
            ...
            n_gpu=self.n_gpu,
            max_env_worker=self.max_env_worker,
        )
    ...
    def train(self):
        ...
        for epoch in range(num_epochs):
            for _, task in enumerate(self.dataset.generate_training_tasks()):
                ...
                time_to_eval = task_count % (EVAL_INTERVAL * self.batch_size) == 0
...
def main():
    ...
    parser.add_argument("--max-response-length", type=int, default=15000,
    ...
    parser.add_argument("--total-training-steps", type=int, default=60,
