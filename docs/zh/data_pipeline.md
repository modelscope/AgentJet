...
在智能体训练中，所有训练数据都必须以**任务（task）**的形式表示，并遵循统一的 Schema。
...
| 场景 | Reader Type | 适用情况 |
|------|-------------|----------|
| **本地 JSONL 文件** | `jsonl_dataset_file` | 您已经准备好 JSONL 格式的任务数据 |
...
**JSONL 格式要求：** 每一行都应是一个符合 Task Schema 的 JSON 对象：
...
<div class="card-grid">
<a href="./task_judger/" class="feature-card"><div class="card-header"><img src="https://api.iconify.design/mdi:check-decagram.svg" class="card-icon card-icon-general" alt=""><h3>任务评判器</h3></div><p class="card-desc">学习如何评估智能体输出并计算奖励。</p></a>
<a href="./data_generation/" class="feature-card"><div class="card-header"><img src="https://api.iconify.design/mdi:creation.svg" class="card-icon card-icon-agent" alt=""><h3>数据生成</h3></div><p class="card-desc">了解如何从文档自动生成训练任务。</p></a>
</div>
